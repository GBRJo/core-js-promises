...
function getPromise(number) {
  return new Promise(function ifNumber(resolve, reject) {
    if (number >= 0) {
      resolve(number);
    } else {
...
function getFirstResolvedPromiseResult(promises) {
  return new Promise((resolve, reject) => {
    ...
    Promise.race(promises).catch(() => {
      const restPromises = promises.filter((prom) => prom !== promises[0]);
      if (restPromises.length === 0) {
        reject(new Error('Error'));
      } else {
        getFirstResolvedPromiseResult(restPromises).then(resolve).catch(reject);
      }
    });
  });
}
